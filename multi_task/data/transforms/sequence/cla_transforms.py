...
class PaddingToMinimumLength(object):
    """
        input: numpy  V * L
    """
    def __init__(self, minimum_length):
        self.minimum_length = abs(minimum_length)   # abs for ensemble prediction

    def __call__(self, sequence):
        length = sequence.shape[-1]
        if self.minimum_length > length:
            diff = self.minimum_length - length
